<!-- Upload  -->
<form id="file-upload-form" action="{{route('admin.projects.imageUpload')}}" class="uploader">
    <input type="hidden" name="project_id" value="{{$projectId}}" />
    <input id="file-upload" type="file" name="fileUpload[{{$i}}]" accept="image/*" multiple />
  ...
  function parseFile(file) {
    ...
    if (isGood) {
        if(imageSize <= fileSizeLimit * 1024 * 1024) {
            ...
            document.getElementById('file-drag').appendChild(img);
        }
    }
    else {
